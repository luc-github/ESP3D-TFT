...
esp_err_t bsp_init(void) {
  ...
  if (esp_lcd_rgb_panel_register_event_callbacks(disp_panel, &cbs, NULL) !=
      ESP_OK) {
 ...
 *
 * This function is responsible for patching display when accessing the file
 * system.
 *
...
/**
 * @brief Revert the display patch when releasing the access of the file system
 * resources.
 *
 * This function revert the display patch when releasing access of the file
 * system resources.
 *
 * @return esp_err_t Returns ESP_OK if the file system resources are
 * successfully released, otherwise returns an error code.
 */
 ...
 * It is responsible for handling the event and performing any necessary
 * actions.
 *
