...
esp_lcd_panel_handle_t st7262_panel_handle = NULL;

esp_lcd_panel_handle_t *get_st7262_panel_handle() {
  return &st7262_panel_handle;
}
...
SemaphoreHandle_t st7262_sem_vsync_end;
SemaphoreHandle_t st7262_sem_gui_ready;
#endif  // DISP_AVOID_TEAR_EFFECT_WITH_SEM

static bool st7262_on_vsync_event(
    esp_lcd_panel_handle_t panel,
    const esp_lcd_rgb_panel_event_data_t *event_data, void *user_data) {
  BaseType_t high_task_awoken = pdFALSE;
#if DISP_AVOID_TEAR_EFFECT_WITH_SEM
  if (xSemaphoreTakeFromISR(st7262_sem_gui_ready, &high_task_awoken) ==
      pdTRUE) {
    xSemaphoreGiveFromISR(st7262_sem_vsync_end, &high_task_awoken);
  }
#endif  // DISP_AVOID_TEAR_EFFECT_WITH_SEM
  return high_task_awoken == pdTRUE;
}

void st7262_flush(lv_disp_drv_t *drv, const lv_area_t *area,
...
#if DISP_AVOID_TEAR_EFFECT_WITH_SEM
  xSemaphoreGive(st7262_sem_gui_ready);
  xSemaphoreTake(st7262_sem_vsync_end, portMAX_DELAY);
#endif  // DISP_AVOID_TEAR_EFFECT_WITH_SEM
...
esp_err_t st7262_init(lv_disp_drv_t *disp_drv) {
  esp3d_log("init st7262 panel");
  if (st7262_panel_handle != NULL) {
    esp3d_log_e("st7262 bus already initialized");
    return ESP_FAIL;
  }
  ...
  st7262_sem_vsync_end = xSemaphoreCreateBinary();
  if (!st7262_sem_vsync_end) {
    esp3d_log_e("Failed to create st7262_sem_vsync_end");
    return ESP_FAIL;
  }
  st7262_sem_gui_ready = xSemaphoreCreateBinary();
  if (!st7262_sem_gui_ready) {
  ...
  ESP_ERROR_CHECK(esp_lcd_new_rgb_panel(&panel_config, &st7262_panel_handle));

  esp3d_log("Register event callbacks");
  esp_lcd_rgb_panel_event_callbacks_t cbs = {
      .on_vsync = st7262_on_vsync_event,
  };
  ESP_ERROR_CHECK(esp_lcd_rgb_panel_register_event_callbacks(
      st7262_panel_handle, &cbs, &disp_drv));

  esp3d_log("Initialize RGB LCD panel");
  ESP_ERROR_CHECK(esp_lcd_panel_reset(st7262_panel_handle));
  ESP_ERROR_CHECK(esp_lcd_panel_init(st7262_panel_handle));
