...
/**
 * @brief Structure representing a LCD command.
 *
 * This structure is used to define a command to be sent to the LCD controller.
 * It contains the command byte, an array of data bytes, and the number of data
 * bytes. The number of data bytes can be set to 0xFF to indicate the end of
 * commands.
 */
typedef struct {
  uint8_t cmd;        ///< The command byte.
  uint8_t data[16];   ///< The array of data bytes.
  uint8_t num_bytes;  ///< The number of data bytes; 0xFF indicates the end of
                      ///< commands.
} lcd_cmd_t;
 ...
 * @param ret_panel Pointer to store the handle of the created LCD panel.
...
err:
  ...
  esp3d_log_e("failed to create ili9341 panel, err=%d", ret);
  return ret;
}
...
static esp_err_t lcd_panel_del(esp_lcd_panel_t *panel) {
  ...
  if (ret == ESP_OK) {
    ...
    esp3d_log_e("failed to delete ili9341 panel, err=%d", ret);
  }
 ...
 *
 * This function initializes the specified LCD panel.
 *
 ...
 *
 * @param panel The LCD panel object.
 ...
 *
 * This function is used to invert the color data of the LCD panel. When the
 * `invert_color_data` parameter is set to `true`, the color data will be
 * inverted. When set to `false`, the color data will remain unchanged.
 *
 * @param panel The pointer to the LCD panel structure.
 * @param invert_color_data A boolean value indicating whether to invert the
 *                          color data.
 * @return `ESP_OK` if the color inversion is successful, otherwise an error
 *         code indicating the cause of the failure.
 */
static esp_err_t lcd_panel_invert_color(esp_lcd_panel_t *panel,
                                        bool invert_color_data) {
 ...
 *
 * @return `ESP_OK` if the axes are successfully swapped.
 ...
 *
 * This function is used to control the power state of the LCD panel. When `off`
 * is set to `true`, the display will be turned off. When `off` is set to
 * `false`, the display will be turned on.
 *
 * @param panel Pointer to the LCD panel structure.
 * @param off   Flag indicating whether to turn the display off (`true`) or on
 * (`false`).
 *
 * @return
