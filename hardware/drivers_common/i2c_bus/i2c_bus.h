...
#define NULL_I2C_MEM_ADDR                                                      \
  0xFF /*!< set mem_address to NULL_I2C_MEM_ADDR if i2c device has no internal \
          address during read/write */
...
esp_err_t i2c_bus_read_bit(i2c_bus_device_handle_t dev_handle,
                           uint8_t mem_address, uint8_t bit_num, uint8_t *data);
 ...
 *
 * @param dev_handle I2C device handle
 * @param mem_address The internal 16-bit reg/mem address to read from, set to
 * NULL_I2C_MEM_ADDR if no internal address.
 * @param data_len Number of bytes to read
 * @param data Pointer to a buffer to save the data that was read
 * @return esp_err_t
