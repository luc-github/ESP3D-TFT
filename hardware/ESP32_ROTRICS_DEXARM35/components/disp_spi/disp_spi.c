 ...
 *      DEFINES
 *********************/
#define SPI_TRANSACTION_POOL_SIZE \
  50 /* maximum number of DMA transactions simultaneously in-flight */

/* DMA Transactions to reserve before queueing additional DMA transactions. A
 * 1/10th seems to be a good balance. Too many (or all) and it will increase
 * latency. */
#define SPI_TRANSACTION_POOL_RESERVE_PERCENTAGE 10
#if SPI_TRANSACTION_POOL_SIZE >= SPI_TRANSACTION_POOL_RESERVE_PERCENTAGE
#define SPI_TRANSACTION_POOL_RESERVE \
  (SPI_TRANSACTION_POOL_SIZE / SPI_TRANSACTION_POOL_RESERVE_PERCENTAGE)
#else
 ...
 *   GLOBAL FUNCTIONS
 **********************/
void disp_spi_add_device_config(spi_host_device_t host,
...
void disp_spi_add_device(spi_host_device_t host) {
  disp_spi_add_device_with_speed(host, DISP_SPI_CLOCK_SPEED_HZ);
}

void disp_spi_add_device_with_speed(spi_host_device_t host,
                                    int clock_speed_hz) {
  esp3d_log("Adding SPI device");
  esp3d_log("Clock speed: %dHz, mode: %d, CS pin: %d", clock_speed_hz,
            DISP_SPI_MODE, DISP_SPI_CS);
  ...
  spi_device_interface_config_t devcfg = {
    ...
    .flags = SPI_DEVICE_NO_DUMMY |
             SPI_DEVICE_HALFDUPLEX, /* dummy bits should be explicitly handled
                                       via DISP_SPI_VARIABLE_DUMMY as needed */
#endif
  };

  disp_spi_add_device_config(host, &devcfg);

  /* create the transaction pool and fill it with ptrs to spi_transaction_ext_t
   * to reuse */
  if (TransactionPool == NULL) {
    TransactionPool = xQueueCreate(SPI_TRANSACTION_POOL_SIZE,
                                   sizeof(spi_transaction_ext_t *));
    assert(TransactionPool != NULL);
    for (size_t i = 0; i < SPI_TRANSACTION_POOL_SIZE; i++) {
      spi_transaction_ext_t *pTransaction =
          (spi_transaction_ext_t *)heap_caps_malloc(
...
void disp_spi_transaction(const uint8_t *data, size_t length,
  ...
  if (flags & DISP_SPI_RECEIVE) {
    assert(out != NULL &&
           (flags & (DISP_SPI_SEND_POLLING | DISP_SPI_SEND_SYNCHRONOUS)));
  ...
  if (flags & DISP_SPI_SEND_POLLING) {
    disp_wait_for_pending_transactions(); /* before polling, all previous
                                             pending transactions need to be
                                             serviced */
    spi_device_polling_transmit(spi, (spi_transaction_t *)&t);
  } else if (flags & DISP_SPI_SEND_SYNCHRONOUS) {
    disp_wait_for_pending_transactions(); /* before synchronous queueing, all
                                             previous pending transactions need
                                             to be serviced */
    ...
    if (uxQueueMessagesWaiting(TransactionPool) == 0) {
      ...
      while (uxQueueMessagesWaiting(TransactionPool) <
             SPI_TRANSACTION_POOL_RESERVE) {
        if (spi_device_get_trans_result(spi, &presult, 1) == ESP_OK) {
          xQueueSend(TransactionPool, &presult,
                     portMAX_DELAY); /* back to the pool to be reused */
        }
      }
    }
    ...
    if (spi_device_queue_trans(spi, (spi_transaction_t *)pTransaction,
                               portMAX_DELAY) != ESP_OK) {
      xQueueSend(TransactionPool, &pTransaction,
                 portMAX_DELAY); /* send failed transaction back to the pool to
                                    be reused */
    }
  }
}
...
void disp_wait_for_pending_transactions(void) {
  ...
  while (uxQueueMessagesWaiting(TransactionPool) <
         SPI_TRANSACTION_POOL_SIZE) { /* service until the transaction reuse
                                         pool is full again */
    if (spi_device_get_trans_result(spi, &presult, 1) == ESP_OK) {
...
void disp_spi_release(void) { spi_device_release_bus(spi); }
