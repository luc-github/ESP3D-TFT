 ...
 *   GLOBAL FUNCTIONS
 **********************/
void tp_spi_add_device_config(spi_host_device_t host,
...
void tp_spi_add_device(spi_host_device_t host) {
  spi_device_interface_config_t devcfg = {
      .clock_speed_hz = TOUCH_SPI_CLOCK_SPEED_HZ,
      .mode = TOUCH_SPI_MODE,
      .spics_io_num = TOUCH_SPI_CS,  // CS pin
      .queue_size = 1,
      .pre_cb = NULL,
      .post_cb = NULL,
      .command_bits = 8,
...
void tp_spi_write_reg(uint8_t* data, uint8_t byte_count) {
  spi_transaction_t t = {
      .length = byte_count * 8, .tx_buffer = data, .flags = 0};
