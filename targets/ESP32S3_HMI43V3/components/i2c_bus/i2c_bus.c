...
i2c_bus_handle_t i2c_bus_create(i2c_port_t port, const i2c_config_t *conf)
{
    ...
    if (s_i2c_bus[port].is_init) {
        ...
        if (i2c_config_compare(port, conf)) {
            esp3d_log("i2c%d has been inited, return handle directly, ref_counter=%d", port, s_i2c_bus[port].ref_counter);
...
esp_err_t i2c_bus_delete(i2c_bus_handle_t *p_bus)
{
    ...
    if ((i2c_bus->ref_counter) > 0) {
        esp3d_log("i2c%d is also handled by others ref_counter=%u, won't be de-inited", i2c_bus->i2c_port, i2c_bus->ref_counter);
        return ESP_OK;
    }
