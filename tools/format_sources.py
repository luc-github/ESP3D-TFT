...
def format_sources():
    ...
    main_dir = os.path.abspath(os.path.normpath(os.path.join(script_dir, '..', 'main')))
    hardware_dir = os.path.abspath(os.path.normpath(os.path.join(script_dir, '..', 'hardware')))

    # Parse all c, h, cpp, and ino files in all directories and subdirectories
    file_paths = []
    for base_dir in [main_dir, hardware_dir]:
