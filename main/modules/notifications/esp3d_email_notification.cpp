...
bool Esp3DNotificationsService::getEmailInformationsFromSettings()
{
    ...
    esp3d_log("Server: %s, port: %s, email: %s, method: %s", _serveraddress.c_str(),_port.c_str(), _settings.c_str(), _method.c_str());
    return true;
}
...
int Esp3DNotificationsService::perform_tls_handshake(mbedtls_ssl_context *ssl)
{
    ...
    while ((ret = mbedtls_ssl_handshake(ssl)) != 0 && ((esp3d_hal::millis()-startTimeout) < EMAIL_TIMEOUT)) {
...
int Esp3DNotificationsService::write_ssl_and_get_response(mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
    int ret;
...
int Esp3DNotificationsService::write_tls_and_get_response(mbedtls_net_context *sock_fd, unsigned char *buf, size_t len)
{
    int ret;
    const size_t DATA_SIZE = 128;
    unsigned char data[DATA_SIZE];
...
{
    if (_token1.length() == 0 || _token2.length()==0  || _settings.length()==0 || _port.length()==0 || _serveraddress.length()==0 || _method.length()==0) {
    ...
    if (!hasError) {
        ...
        ret = mbedtls_base64_encode((unsigned char *) base64_buffer, sizeof(base64_buffer),
                                    &base64_len, (unsigned char *) _token1.c_str(), strlen(_token1.c_str()));
        if (ret != 0) {
    ...
    if (!hasError) {
        ...
        ret = mbedtls_base64_encode((unsigned char *) base64_buffer, sizeof(base64_buffer),
                                    &base64_len, (unsigned char *) _token2.c_str(), strlen(_token2.c_str()));
        if (ret != 0) {
            esp3d_log_e("Error in mbedtls encode! ret = -0x%x", -ret);
            hasError = true;
        }
