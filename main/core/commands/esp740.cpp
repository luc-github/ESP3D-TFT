...
// List SD Filesystem
///[ESP740]<Root> json=<no> pwd=<user/admin password>
void Esp3DCommands::ESP740(int cmd_params_pos, esp3d_msg_t *msg) {
  bool hasError = false;
  ...
  if (sd.accessFS()) {
    DIR *dir = sd.opendir(tmpstr.c_str());
    ...
    if (dir) {
      ...
      sd.getSpaceInfo(&totalSpace, &usedSpace, &freeSpace, true);
