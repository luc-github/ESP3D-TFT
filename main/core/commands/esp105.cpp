...
void Esp3DCommands::ESP105(int cmd_params_pos,esp3d_msg_t * msg)
{
    ...
    if (tmpstr.length()==0) {
        const esp3d_setting_desc_t * settingPtr = esp3dTFTsettings.getSettingPtr(esp3d_ap_ssid);
        if (settingPtr) {
            ok_msg = esp3dTFTsettings.readString(esp3d_ap_ssid,out_str, settingPtr->size);
        } else {
            hasError = true;
            error_msg="This setting is unknown";
        }
    } else {
        esp3d_log("got %s param for a value of %s, is valid %d", tmpstr.c_str(),tmpstr.c_str(), esp3dTFTsettings.isValidStringSetting(tmpstr.c_str(), esp3d_ap_ssid));
        if (esp3dTFTsettings.isValidStringSetting(tmpstr.c_str(), esp3d_ap_ssid)) {
            esp3d_log("Value %s is valid",tmpstr.c_str());
            if (!esp3dTFTsettings.writeString (esp3d_ap_ssid, tmpstr.c_str())) {
                hasError = true;
