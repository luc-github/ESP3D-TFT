...
void Esp3DCommands::ESP105(int cmd_params_pos, Esp3dMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    const Esp3dSettingDescription* settingPtr =
        esp3dTFTsettings.getSettingPtr(esp3d_ap_ssid);
    if (settingPtr) {
      ok_msg =
          esp3dTFTsettings.readString(esp3d_ap_ssid, out_str, settingPtr->size);
    } else {
      hasError = true;
    ...
    esp3d_log(
        ...
        tmpstr.c_str(),
        esp3dTFTsettings.isValidStringSetting(tmpstr.c_str(), esp3d_ap_ssid));
