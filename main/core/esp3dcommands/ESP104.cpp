...
void Esp3DCommands::ESP104(int cmd_params_pos,esp3d_msg_t * msg)
{
    ...
    if (tmpstr.length()==0) {
        byteValue = esp3dTFTsettings.readByte(esp3d_fallback_mode);
        ...
        esp3d_log("got %s param for a value of %d, is valid %d", tmpstr.c_str(),byteValue, esp3dTFTsettings.isValidByteSetting(byteValue, esp3d_fallback_mode));
        if (esp3dTFTsettings.isValidByteSetting(byteValue, esp3d_fallback_mode)) {
            esp3d_log("Value %d is valid",byteValue);
            if (!esp3dTFTsettings.writeByte (esp3d_fallback_mode, byteValue)) {
                hasError = true;
