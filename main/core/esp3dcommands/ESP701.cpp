...
void Esp3DCommands::ESP701(int cmd_params_pos,esp3d_msg_t * msg)
{
    ...
    esp3d_gcode_host_state_t status = gcodeHostService.getStatus();
    esp3d_gcode_host_error_t errorNum = gcodeHostService.getErrorNum();
    if (tmpstr.length()==0) {
        if (status == ESP3D_HOST_NO_STREAM) {
            if (json) {
                ok_msg = "{\"status\":\"no stream\"";
                if (errorNum!=ESP3D_NO_ERROR_STREAM) {
                    ok_msg+="\",\"code\":\"";
                    ok_msg+=std::to_string(errorNum);
                    ok_msg+="\"";
                }
                ok_msg +="}";
            } else {
            ...
            esp3d_script_t * script = gcodeHostService.getCurrentScript();
            if (script) {
                if (status == ESP3D_HOST_PAUSED_STREAM) {
                    if (json) {
                ...
                if (json) {
                    ...
                    ok_msg +="\",\"type\":\"";
                    ok_msg+=std::to_string(script->type);
                    if(script->type !=ESP3D_TYPE_SCRIPT_STREAM) {
                        ok_msg +="\",\"name\":\"";
