...
void Esp3DCommands::ESP161(int cmd_params_pos,esp3d_msg_t * msg)
{
    ...
    if (tmpstr.length()==0) {
        intValue = esp3dTFTsettings.readUint32(esp3d_ws_port);
        ok_msg=std::to_string(intValue);
    } else {
        intValue = atoi(tmpstr.c_str());
        esp3d_log("got %s param for a value of %ld, is valid %d", tmpstr.c_str(),intValue, esp3dTFTsettings.isValidIntegerSetting(intValue, esp3d_ws_port));
        if (esp3dTFTsettings.isValidIntegerSetting(intValue, esp3d_ws_port)) {
            esp3d_log("Value %ld is valid",intValue);
            if (!esp3dTFTsettings.writeUint32 (esp3d_ws_port, intValue)) {
                hasError = true;
