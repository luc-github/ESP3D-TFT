...
void Esp3DCommands::ESP950(int cmd_params_pos,esp3d_msg_t * msg)
{
    ...
    esp3d_clients_t client_output = (esp3d_clients_t)esp3dTFTsettings.readByte(esp3d_output_client);
    if (tmpstr.length()==0) {
        ...
        if ((usbclient && !serialclient) || (serialclient && !usbclient)) {
            esp3d_clients_t newoutput = serialclient?SERIAL_CLIENT:USB_SERIAL_CLIENT;
