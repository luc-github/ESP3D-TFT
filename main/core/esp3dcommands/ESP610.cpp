...
void Esp3DCommands::ESP610(int cmd_params_pos,esp3d_msg_t * msg)
{
    ...
    const esp3d_setting_index_t settingIndex [] = { esp3d_notification_type,
    ...
    if (tmpstr.length()==0) {
        ...
        ok_msg += esp3dTFTsettings.readString(esp3d_notification_token_setting, buffer, SIZE_OF_SETTING_NOFIFICATION_TS);
        if (json) {
            ok_msg += "\"}";
        } else {
            ok_msg +="\n";
        ...
        for (uint8_t i = 0; i < cmdListSize; i++) {
            ...
            if (tmpstr.length()!=0) {
                ...
                case esp3d_notification_token_setting:
                    if (esp3dTFTsettings.isValidStringSetting(tmpstr.c_str(), settingIndex[i])) {
                ...
                case esp3d_auto_notification:
                    if (strcasecmp(tmpstr.c_str(), "YES")==0 || strcasecmp(tmpstr.c_str(), "1")==0||strcasecmp(tmpstr.c_str(), "ON")==0||strcasecmp(tmpstr.c_str(), "ENABLED")==0) {
                        val=1;
                    } else if (strcasecmp(tmpstr.c_str(), "NO")==0 || strcasecmp(tmpstr.c_str(), "0")==0||strcasecmp(tmpstr.c_str(), "OFF")==0 || strcasecmp(tmpstr.c_str(), "DISABLED")==0) {
                        val=0;
                    } else {
                        hasError = true;
