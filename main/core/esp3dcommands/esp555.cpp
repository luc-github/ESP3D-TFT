...
void Esp3DCommands::ESP555(int cmd_params_pos,esp3d_msg_t * msg)
{
    ...
    if (tmpstr.length()==0) {
        ...
        esp3d_log("got %s param for a value of %s, is valid %d", tmpstr.c_str(),tmpstr.c_str(), esp3dTFTsettings.isValidStringSetting(tmpstr.c_str(), esp3d_user_password));
        if (esp3dTFTsettings.isValidStringSetting(tmpstr.c_str(), esp3d_user_password)) {
